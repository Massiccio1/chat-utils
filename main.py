...
def save_data(id, data):
    with open(f'data/{id}.npy', 'wb') as f:
        np.save(f, data)
        
def load_data(id):
    data = np.empty()
    with open(f'data/{id}.npy', 'rb') as f:
        data = np.load(f)
    return data

def get_peaks(id):
    timeline_density=np.empty()
    if not os.path.isfile(f"data/{id}.csv"):    #numpy non salvato
        ...
        save_data(id,timeline_density)
    else:
        timeline_density=load_data(id)
    ...
    peaks2, _ = find_peaks(timeline_density[:,1], prominence=0.5)
    ...
    plt.plot(peaks2+ time0, timeline_density[:,1][peaks2], "or")
